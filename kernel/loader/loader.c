...
int is_load[TASK_MAXNUM] = {0}; // 记录每个任务是否已经加载过，0表示未加载，1表示已加载
    ...
    for (int i = 0; i < TASK_MAXNUM; i++) {
        if (tasks[i].block_nums > 0 && strcmp(task_name, tasks[i].task_name) == 0) {
            ...
            if (is_load[i] == 0) {
                ...
                memcpy((uint8_t *)(uint64_t)(mem_addr), (uint8_t *)(uint64_t)(TMP_MEM_BASE + (tasks[i].start_addr - start_sec * 512)), tasks[i].block_nums * 512);
            }
            return mem_addr;
        }
    }
